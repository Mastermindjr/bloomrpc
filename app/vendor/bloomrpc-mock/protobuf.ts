 ...
 * https://github.com/bloomrpc/bloomrpc-mock
 */
import * as fs from 'fs';
import { GrpcObject, loadPackageDefinition } from '@grpc/grpc-js';
import { load as loadPackageDef } from '@grpc/proto-loader';
// @ts-ignore
...
export async function fromFileName(protoPath: string, includeDirs?: string[]): Promise<Proto> {
  ...
  const packageDefinition = await loadPackageDef(path.basename(protoPath), {
    keepCase: true,
    longs: String,
    enums: String,
    defaults: true,
    ...
    if (nestedNamespaceTypes) {
      Object.keys(nestedNamespaceTypes).forEach(nestedTypeName => {
        ...
        if (nestedType instanceof Service) {
          ...
          onService(nestedType, lodashGet(ast, serviceName), fullyQualifiedServiceName);
        }
      });
    }
  });
...
export function serviceByName(root: Root, serviceName: string): Service {
  if (!root.nested) {
